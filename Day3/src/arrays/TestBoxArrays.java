...
//		5. Can you use for-each loop , to double box widths for the boxes having volume < 100 ? 
		...
		System.out.println("Doubling the width");
		
		for(Box b:boxes) {
			if(b.getVolume()<100) {
				// Double the width
				b.setWidth(b.getWidth() * 2);
				System.out.println("After doubling - " + b.getBoxDimensions() + 
								   ", Volume: " + b.getVolume());
			}
		}
		
		// Display final box details
		System.out.println("\nFinal box details:");
		for(Box b : boxes) {
			System.out.println(b.getBoxDimensions() + ", Volume: " + b.getVolume());
		}
