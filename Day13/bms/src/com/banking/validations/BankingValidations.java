...
public class BankingValidations {
	...
	public static BankAccount validateAllInputs(int accountNumber, double balance, String customerName, String phoneNumber,String dob,String acType,double rateOrLimit,List<BankAccount>accountList)throws BankingException 
	{
		...
		if(type.equals(AccountType.SAVING)) {
			 if (rateOrLimit <= 0) {
	                throw new BankingException("Interest rate must be positive for saving accounts");
	            }
	            return new SavingAccount(accountNumber, balance, customerName, phoneNumber, 
	                                    rateOrLimit, birthDate, type);
	        } else {
	            if (rateOrLimit < 0) {
	                throw new BankingException("Overdraft limit cannot be negative for current accounts");
	            }
